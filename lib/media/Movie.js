...
class Movie extends Content {
   ...
   */
  static GetAll() {
    // TODO: Inner join metadata stuff
    return new Promise((resolve, reject) => {
      db.any('SELECT * FROM movie').then(data => {
        // Convert BigInt in postgres to int
        const result = data.map(movie => new Movie(parseInt(movie.id)));
        resolve(result);
      }).catch(error => {
  ...
  getLanguages() {
    return new Promise(resolve => {
      db.any('SELECT language, id, stream_index, codec FROM movie_language WHERE movie_id = $1', [this.#movieId]).then(result => {
        resolve(result);
      });
    });
  }
