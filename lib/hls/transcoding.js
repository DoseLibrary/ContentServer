...
class Transcoding {
  ...
  getCpuVideoCodec(h265) {
    return h265 ? "libx265" : "libx264";
  }

  getVideoCodec(useH265) {
    if (this.gpuTranscoding) {
      return this.getGpuVideoCodec();
    } else {
      return this.getCpuVideoCodec(useH265);
    }
  }
  ...
  getHdrOutputOptions(hdrData, useH265) {
    const options = [
      `-pix_fmt ${hdrData.pixFmt}`
    ];
    // Can only use color settings when using libx265
    if (useH265 && hdrData.colorInfoExists) {
      options.push(
        `-x265-params colorprim=${hdrData.colorPrimaries}:transfer=${hdrData.colorTransfer}:colormatrix=${hdrData.colorSpace}`
      );
    }
    return options;
  }
  ...
  getOutputOptions(crf, preset) {
    ...
    if (!this.gpuTranscoding) {
      return options.concat(this.getCpuOutputOptions(preset));
    }
    return options;
  }
  ...
  getCpuInputOptions(threads) {
    return [`-threads ${threads}`];
  }

  getInputOptions(threads) {
    const options = [
      '-y',
      '-loglevel verbose',
      '-copyts', // Fixes timestamp issues (Keep timestamps as original file)
    ...
    if (this.gpuTranscoding) {
      ...
      return options.concat(this.getCpuInputOptions(threads));
    }
  }
   ...
   * @param {} hdrData 
   * @param {boolean} useH265 
   * @param {{crf: number}} settings 
   * @returns 
   */
  start(quality, output, audioStreamIndex, hdrData, useH265, settings) {
    ...
    return new Promise(resolve => {
      ...
      outputOptions = outputOptions.concat(this.getHdrOutputOptions(hdrData, useH265));
      outputOptions = outputOptions.concat(this.getQualityParameter(quality));

      const inputOptions = this.getInputOptions(settings.threads);
      ...
      this.ffmpegProc = ffmpeg(this.filePath)
        .withVideoCodec(this.getVideoCodec(useH265))
        .withAudioCodec("aac")
