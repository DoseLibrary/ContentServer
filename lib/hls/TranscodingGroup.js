...
class TranscodingGroup {
     ...
     */
    async start(quality, output, audioStreamIndex, hdrData, useH265) {
        this.output = output;
        this.quality = quality;

        const settings = new Settings();
        const transcodingSettings = useH265 ? await settings.getH265Settings() : await settings.getH264Settings();

        let promises = [];
        promises.push(this.fastTranscoding.start(quality, output, audioStreamIndex, hdrData, useH265, transcodingSettings));
        if (this.slowTranscoding) {
            promises.push(this.slowTranscoding.start(quality, output, audioStreamIndex, hdrData, useH265, transcodingSettings));
        }
        return promises;
    }
}
