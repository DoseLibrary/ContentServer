...
class TranscodingGroup {
     ...
     */
    start(quality, output, audioStreamIndex, audioTranscoding, hdrData, useH265) {
        this.output = output;
        this.quality = quality;

        let promises = [];
        promises.push(this.fastTranscoding.start(quality, output, audioStreamIndex, audioTranscoding, hdrData, useH265));
        if (this.slowTranscoding) {
            promises.push(this.slowTranscoding.start(quality, output, audioStreamIndex, audioTranscoding, hdrData, useH265));
        }
        return promises;
    }
}
