...
class HlsManager {
    ...
    startTranscoding(content, quality, startSegment, groupHash, audioStreamIndex, audioTranscoding, hdrData, useH265, user) {
        return new Promise(resolve => {
            Promise.all([content.getFilePath(), config.getConfig()]).then(([filePath, conf]) => {
                ...
                const promises = transcodingGroup.start(quality, output, audioStreamIndex, audioTranscoding, hdrData, useH265);
