...
class HlsManager {
    ...
    startTranscoding(content, quality, startSegment, groupHash, audioStreamIndex, hdrData, useH265, user) {
        return new Promise(resolve => {
            Promise.all([content.getFilePath(), config.getConfig()]).then(async ([filePath, conf]) => {
                ...
                const promises = await transcodingGroup.start(quality, output, audioStreamIndex, hdrData, useH265);
