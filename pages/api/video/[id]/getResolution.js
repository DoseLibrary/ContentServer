...
export default async (req, res) => {
  return new Promise(async (resolve, reject) => {
    ...
    const allAudioCodecsSupported = languages.every(language => browser.audioCodecSupported(language.codec));
    const directPlay = browser.videoCodecSupported(resolution["codec"]) && allAudioCodecsSupported && !userAgent.isIphone;
    logger.DEBUG(`Diretplay supported: ${directPlay} for content with id ${req.query.id}`);
    res.status(200).json({
